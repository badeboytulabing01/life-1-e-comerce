 <div class="row">
      <?php 
       ...
       ?>
        <div class="col-md-12">
           <?php $view_product = mysqli_query($conn, "SELECT * FROM tbl_buying where user_id='$customerID' and status='toreceive'"); ?>
          <table class="table">
         <thead>
             <div class="d-flex">
                 <h5 class="text-uppercase me-1">To</h5>
                 <h5 class="text-uppercase text-success">Receive</h5>
              </div>
           <tr>
             <th>Product</th>
              <th>Unit Price</th>
 ...
 <tbody>
    ...
    $view_totalSale = mysqli_query($conn, "SELECT status, user_id, SUM(price*qty+shipping_fee) as total_sale FROM tbl_buying WHERE user_id='$user_id' and status='toreceive'"); ?>
     <?php while ($row = mysqli_fetch_assoc($view_totalSale)):
     $status = $row["status"];?>
    <td class="d-flex">
         <div class="row" id="check_out">
          <div class="col-md-12">          
           <span class="me-3 mt-2"> Total: ₱<?php $total_sale = $row["total_sale"];
       ...
       <?php 
               if ($status>="toreceive") {
                 echo
                  "
                    <form method='POST' action='to_receive.php'>
                      <input type='hidden' name='user_id' value='$user_id'> 
                        <button type='submit' name='complete' id='complete' class='btn btn-outline-primary'>
                           Receive </button>
                    </form>
                  ";
               }else{
